...
LABELS = {
    "en": {
        ...
        "edition": "Samsung, Honor & Xiaomi Edition", # Updated
    ...
    "ar": {
        ...
        "edition": "إصدار سامسونج، هونور، شاومي", # Updated
...
# ========== THEMES ==========
DARK_BLUE_ACCENT = "#003366" # Darker, more indigo/navy
DARK_BLUE_ACCENT2 = "#002244" # Even darker for hover/active
...
class ModernButton(tk.Button):
    def __init__(self, master, text, command, theme, width=25, height=2, icon_path=None, icon_size=(16,16), state=tk.NORMAL, **kwargs): 
        ...
        if icon_path and PIL_AVAILABLE:
            try:
                # Use load_image which handles path correctly
                self.icon_image = load_image(icon_path, size=icon_size)
                if self.icon_image:
                    app_images[f"btn_{text}_{icon_path}"] = self.icon_image # Store to prevent GC
            except Exception as e:
...
class LoginWindow(tk.Toplevel):
    def __init__(self, parent, app_controller):
        ...
        app_images['login_banner'] = load_image("login_banner.png", size=(380, 100)) 
        ...
        tk.Label(main_frame, text=self.labels.get("login_title", "Login"), font=TITLE_FONT, bg=self.theme["BG"], fg=self.theme.get("TITLE_FG", self.theme["ACCENT"])).pack(pady=(0, 10))

        if app_images.get('login_banner'):
            banner_label = tk.Label(main_frame, image=app_images['login_banner'], bg=self.theme["BG"], cursor="hand2")
            banner_label.image = app_images['login_banner'] 
            banner_label.pack(pady=(5,15))
...
class AppController:
    def __init__(self):
        ...
        # Try to load telegram icon early for buttons
        app_images['telegram_icon'] = load_image("telegram_icon.png", size=(18,18))


        try:
            webbrowser.open("https://ultimat-unlock.com/") # This was requested to open at start
...
class UltimateDeviceTool(tk.Frame):
    ...
    def _apply_styles(self):
        ...
        self.style.map("TNotebook.Tab",
                       ...
                       relief=[("selected", "flat")]) # Ensure selected tab is also flat
    ...
    def _build_ui(self):
        ...
        ModernButton(bottom_buttons_frame, text=self.labels.get("btn_telegram_channel", "Telegram Channel"), 
                     ...
                     theme=self.theme, width=18, height=1, icon_path="telegram_icon.png", icon_size=(18,18), padx=5, pady=2).pack(side=tk.LEFT, padx=(10,5), pady=2) # Added left padx
        ModernButton(bottom_buttons_frame, text=self.labels.get("btn_contact_support", "Contact Support"), 
                     ...
                     theme=self.theme, width=18, height=1, icon_path="telegram_icon.png", icon_size=(18,18), padx=5, pady=2).pack(side=tk.LEFT, padx=5, pady=2)

        self.status_bar.pack(side=tk.BOTTOM, fill=tk.X)
        bottom_buttons_frame.pack(side=tk.BOTTOM, fill=tk.X, pady=(5,2), padx=0) # Removed padx from frame
    ...
    def _build_device_status_widgets(self, parent_frame):
        # Clear previous widgets if rebuilding
        ...
        refresh_btn = ModernButton(status_frame, text=self.labels.get("btn_refresh_status", "Refresh"), 
                                   ...
                                   theme=self.theme, width=10, height=1, padx=5, pady=1) # Smaller button
        refresh_btn.pack(side=tk.RIGHT, anchor=tk.SE, padx=(10,0), pady=(5,0))
        
        # TODO: Initial population of status vars
        self.action_refresh_device_status(initial_load=True)


    def action_refresh_device_status(self, initial_load=False):
        # This is a placeholder for now. It should trigger a quick device info read.
        # For now, it just logs and uses the main ADB status check for connection.
        if not initial_load and self.log_panel: # Don't log on initial auto-load
            self.log_panel.log("Refreshing device status panel...", "info", include_timestamp=False)
        
        # Simplified update based on existing ADB check for connection status
        # A more robust solution would involve a quick `adb get-state` and `adb shell getprop` for key items.
        
        def update_from_adb_status():
            adb_full_status = self.status_bar.cget("text") # Get text from main status bar
            ...
            # TODO: Implement actual fetching of model, android, root for the status panel
            # For now, they remain "Unknown" or their last set value.
            # Example of how you might trigger a lightweight info fetch:
            # self.get_lightweight_device_info_for_status_panel()

        if self.winfo_exists(): # Ensure widget exists before scheduling 'after'
            self.after(100, update_from_adb_status) # Small delay to ensure status_bar might have updated
    ...
    def fetch_and_log_device_info(self, operation_label_key_on_success, callback_after_info_and_op=None, next_operation_command=None, next_operation_name=""):
        ...
        def _after_all_props_fetched_for_operation(final_props_dict_internal_keys):
            ...
            if self.winfo_exists(): # Ensure UltimateDeviceTool is still around
                ...
                # Root state needs specific handling
                ...
                # Connection status is handled by the main ADB checker for the status bar, but could be set here too.
    ...
    def _rebuild_ui(self):
        ...
        # Preserve device status panel if it exists
        ...
        # Restore device status panel values
        ...
        elif hasattr(self, 'device_status_panel'): # If vars were not there, try initial refresh
            self.action_refresh_device_status(initial_load=True)
...
class XiaomiTab(ttk.Frame):
    def __init__(self, parent_notebook, master_app: UltimateDeviceTool):
        ...
        ModernButton(adb_col2, text=self.labels.get("btn_xiaomi_enable_diag_root", "Enable Diag (ROOT)") + " *",
                                   command=lambda: messagebox.showinfo("Info", "Enable Diag (ROOT) is a placeholder for specific Xiaomi Diag enabling commands, which usually require root and device-specific knowledge.", parent=self.master_app.master), theme=self.theme, width=30).pack(pady=5, anchor=tk.W)
...
if __name__ == "__main__":
    if not PIL_AVAILABLE:
        # root_pil_check = tk.Tk()
        # root_pil_check.withdraw()
        # messagebox.showwarning("Dependency Warning", "Pillow library is not installed (pip install Pillow). Image features will be limited or unavailable.")
        # root_pil_check.destroy()
        log_to_file_debug_globally("Pillow not installed. UI may not display images correctly.", "CRITICAL")
        # Allow to run, but images might not show. For critical images, could sys.exit.
