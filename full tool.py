...
FIXED_PASSWORD = "admin" # Changed as per user request
...
LABELS = {
    "en": {
        ...
        "tab_app_manager": "App Manager", # New Tab
        "log": "Operation Log",
        ...
        "frp_reset_warning_message": "This will attempt a series of common ADB commands to reset FRP. These commands are not guaranteed to work on all devices or Android versions and may require specific device states (e.g., USB Debugging enabled). Proceed with caution.",
        ...
        "login_window_title": "Ultimat-Unlock V2.2.2", # Changed to match new login screen
    ...
    "ar": {
        ...
        "tab_app_manager": "مدير التطبيقات", # New Tab Arabic
        "log": "سجل العمليات",
        ...
        "frp_reset_warning_message": "سيقوم هذا الإجراء بمحاولة تنفيذ سلسلة من أوامر ADB الشائعة لإزالة قفل FRP. هذه الأوامر ليست مضمونة على جميع الأجهزة أو إصدارات أندرويد وقد تتطلب حالات معينة للجهاز (مثل تفعيل تصحيح USB). تعامل بحذر.",
        "screen_lock_reset_warning_title": "محاولة إزالة قفل الشاشة",
        "screen_lock_reset_warning_message": "يحاول هذا الإجراء إزالة ملفات قفل الشاشة. يتطلب عادةً صلاحيات الروت ومن غير المرجح أن يعمل على إصدارات أندرويد الحديثة بسبب التشفير. قد يحدث فقدان للبيانات. تعامل بحذر شديد.",
        "context_cut": "قص",
        "context_copy": "نسخ",
        "context_paste": "لصق",
        ...
        "login_window_title": "Ultimat-Unlock V2.2.2", # Changed
...
class ModernButton(tk.Button):
    def __init__(self, master, text, command, theme, width=25, height=2, icon_path=None, icon_size=(16,16), state=tk.NORMAL, **kwargs):
        ...
        super().__init__(
            ...
            padx=kwargs.pop('padx', 5 if self.icon_image else 10),
            pady=kwargs.pop('pady', 2 if self.icon_image else 5),
        ...
        if state == tk.NORMAL:
            ...
            self.config(bg=kwargs.get('disabledbackground', theme.get("GROUP_BG", "#2C313A")),
...
class LogPanel(tk.Frame):
    def __init__(self, master, theme, labels, db_logger=None, tk_root=None, app_controller=None):
        ...
        self.text = tk.Text(self, height=20, font=LOG_FONT, state=tk.DISABLED, # Reduced height from 25 to 20
...
class LoginWindow(tk.Toplevel):
    def __init__(self, parent, app_controller):
        ...
        self.title_text = self.labels.get("login_window_title", "Ultimat-Unlock V2.2.2") # Updated title
...
class AppController:
    def __init__(self):
        ...
        self.lang = "en" # Default language set to English
        ...
        app_images['telegram_icon'] = load_image("telegram_icon.png", size=(18,18)) # Default size
        app_images['telegram_icon_small'] = load_image("telegram_icon.png", size=(16,16)) # For smaller buttons

        try:
            log_to_file_debug_globally("Website auto-open skipped.")
    ...
    def show_main_app(self):
        ...
        if self.main_app_window is None:
            ...
            else: # Recreate if somehow destroyed
                 self.main_app_window = UltimateDeviceTool(master_tk_instance=self.root, app_controller=self)
...
class UltimateDeviceTool(tk.Frame):
    def __init__(self, master_tk_instance, app_controller):
        ...
        self.master.geometry("1280x760") # Adjusted height slightly as log panel is smaller
        self.master.wm_minsize(1000, 680) # Adjusted minsize
    ...
    def _build_ui(self):
        ...
        green_button_bg = "#00B050"  # A nice green
        green_button_fg = "#FFFFFF"  # White text on green
        green_button_active_bg = "#008C40" # Darker green for hover/active

        # Using the 'telegram_icon_small' loaded in AppController
        ModernButton(bottom_buttons_frame, text=self.labels.get("btn_telegram_channel", "Telegram Channel"),
                     ...
                     theme=self.theme, width=18, height=1, icon_path="telegram_icon.png", icon_size=(16,16), # Explicitly pass path
                     padx=4, pady=1, font=("Segoe UI", 9, "bold"), # Slightly smaller font
        ...
        body.add(left_area_container, weight=3) # Gave more weight to left
        body.add(right_area_container, weight=1) # Less weight to right (log panel)
        ...
        self.device_status_panel = tk.LabelFrame(left_area_container,
                                            ...
                                            padx=10, pady=10, relief="groove", bd=1)
        ...
        tabs_to_add = [
            ...
            (AppManagerTab, "tab_app_manager") # Added App Manager Tab
        ]
    ...
    def _build_device_status_widgets(self, parent_frame):
        ...
        status_frame = tk.Frame(parent_frame, bg=self.theme.get("DEVICE_STATUS_BG", self.theme["GROUP_BG"]))
        ...
        for i, (label_text, var) in enumerate(info_map):
            row_frame = tk.Frame(status_frame, bg=self.theme.get("DEVICE_STATUS_BG", self.theme["GROUP_BG"]))
            row_frame.pack(fill=tk.X, pady=1)

            tk.Label(row_frame, text=label_text, font=DEVICE_STATUS_FONT_LABEL,
                     bg=self.theme.get("DEVICE_STATUS_BG", self.theme["GROUP_BG"]),
                     fg=self.theme.get("DEVICE_STATUS_FG", self.theme["FG"]), anchor="w").pack(side=tk.LEFT, padx=(0,5))

            tk.Label(row_frame, textvariable=var, font=DEVICE_STATUS_FONT_VALUE,
                     bg=self.theme.get("DEVICE_STATUS_BG", self.theme["GROUP_BG"]),
                     fg=self.theme.get("ACCENT", DARK_BLUE_ACCENT), anchor="w").pack(side=tk.LEFT)

        refresh_btn = ModernButton(status_frame, text=self.labels.get("btn_refresh_status", "Refresh"),
                                   command=self.action_refresh_device_status,
                                   theme=self.theme, width=10, height=1, padx=5, pady=1)
        refresh_btn.pack(side=tk.RIGHT, anchor=tk.SE, padx=(10,0), pady=(5,0))
    ...
    def action_refresh_device_status(self, initial_load=False):
        ...
        def update_from_adb_status():
            if not self.winfo_exists() or not hasattr(self, 'status_bar') or not self.status_bar.winfo_exists(): return
            adb_full_status = self.status_bar.cget("text")
            conn_status_text = self.labels.get("status_unknown", "Unknown")
            if self.labels.get("adb_status_connected") in adb_full_status:
                conn_status_text = "ADB Connected"
            elif self.labels.get("adb_status_not_connected") in adb_full_status:
    ...
    def execute_command_async(self, command_list, operation_name="Operation", callback_on_finish=None, is_part_of_sequence=False, is_info_gathering=False):
        ...
        def _command_thread():
            ...
            try:
                ...
                stdout, stderr = process.communicate(timeout=120) # Consider making timeout configurable or longer for some ops
                ...
                if process and process.returncode is not None and process.returncode < 0 : # Negative return code might indicate termination
    ...
    def _handle_command_result(self, result):
        ...
        if is_info_gathering and "error" not in result and result.get("return_code") == 0:
            # For successful info gathering, usually no direct log to user panel unless it's the main op
            pass
        elif is_part_of_sequence: # Log steps of a sequence
            if "error" in result:
                ...
                stdout = result.get("stdout", "").strip()
                # Only log non-empty, non-generic success messages for steps
                ...
                # else: log_method(f"Step OK: {operation_name}", "info", indent=1, include_timestamp=False) # If you want to log all successful steps
        else: # Main operation logging
            if "error" in result:
                ...
                if not operation_name.startswith("Get Property"): # Avoid logging every single property get
                    if return_code == 0:
                        ...
                        # Log stderr as warning even on success, as some tools output info there
                        if stderr.strip():
        ...
        # Stop progress bar and re-enable cancel button ONLY if it's not part of a sequence and not just info gathering
        if log_panel_available and not is_part_of_sequence and not is_info_gathering:
    ...
    def action_cancel_operation(self):
        if self.current_popen_process and self.current_popen_process.poll() is None:
            try:
                self.current_popen_process.terminate() # Send SIGTERM
                # Optionally, after a short timeout, send SIGKILL if still running: self.current_popen_process.kill()
            ...
            self._update_cancel_button_state(enable=False) # Ensure button is disabled if no op
    ...
    def fetch_and_log_device_info(self, operation_label_key_on_success, callback_after_info_and_op=None, next_operation_command=None, next_operation_name=""):
        ...
        def _after_all_props_fetched_for_operation(final_props_dict_internal_keys):
            ...
            if self.winfo_exists() and hasattr(self, 'status_vars'):
                self.status_vars["model"].set(collected_props_display_keys.get("Model", self.labels.get("status_unknown")))
                self.status_vars["android"].set(collected_props_display_keys.get("Android Version", self.labels.get("status_unknown")))
                root_val = collected_props_display_keys.get("Root State", "N/A")
                if root_val == "0": self.status_vars["root"].set("Rooted!")
                elif root_val == "1": self.status_vars["root"].set("No Root!")
        ...
        def _after_next_operation_completed(result):
            # Main operation logging (success/fail) handled by _handle_command_result
            # This callback is for any post-operation logic
            if callback_after_info_and_op:
                callback_after_info_and_op({"return_code": result.get("return_code", -1),
                                            "device_info": collected_props_display_keys,
                                            "operation_result": result})
            # Ensure progress bar stops and button disables if this was the end of main op
            if log_panel_available:
                if self.log_panel.progress_bar.running : self.log_panel.progress_bar.stop()
                self._update_cancel_button_state(enable=False)
    ...
    def _rebuild_ui(self):
        ...
        if hasattr(self, 'notebook') and self.notebook.winfo_exists():
            try:
                if self.notebook.tabs(): # Check if tabs exist
        ...
        elif hasattr(self, 'device_status_panel'): # If status_vars were not there before, refresh
            self.action_refresh_device_status(initial_load=True)
...
class SamsungTab(ttk.Frame):
    ...
    def _execute_next_frp_step(self):
        ...
        else: # All steps completed or one failed and we decided to stop (though current logic continues)
            if self.master_app.log_panel and self.master_app.log_panel.winfo_exists():
                ...
                # Ensure progress bar stops and button is disabled
    ...
    def _frp_step_callback(self, result):
        ...
        if not is_last_step:
            self._execute_next_frp_step()
        else: # This is the last step, _execute_next_frp_step will handle final logging.
            self._execute_next_frp_step() # Call it one last time to trigger the 'else' block there
    ...
    def action_reset_screenlock_adb(self):
        if messagebox.askokcancel(
            ...
            current_sl_step_ref = [0] # Use a list to pass by reference
    ...
    def action_open_browser_adb(self):
        ...
        elif url is not None: # User provided an empty string
        ...
        else: # User cancelled the dialog (url is None)
...
class HonorTab(ttk.Frame):
    ...
    def action_honor_reboot_edl(self):
        command = ["fastboot", "oem", "edl"] # Common command, might vary
    ...
    def action_honor_wipe_data_cache(self):
        if messagebox.askyesno("Confirm Wipe", "Are you sure you want to wipe data and cache on this Honor device? This will erase all user data.", parent=self.master_app.master):
            ...
            commands_wipe = [
                ...
                (["fastboot", "erase", "userdata"], "Honor Wipe Userdata") # or 'fastboot -w' for some
            ]
    ...
    def action_honor_remove_frp(self):
        ...
        command = ["fastboot", "oem", "frp-unlock", frp_key] # This is a guess, actual command can vary wildly
        self.master_app.execute_command_async(command, operation_name=op_name)
...
class XiaomiTab(ttk.Frame):
    ...
    def action_xiaomi_fastboot_unlock(self):
        if messagebox.askyesno("Confirm Unlock", "Are you sure you want to unlock the bootloader? This will erase all user data and may void warranty. For Xiaomi, this often requires Mi Unlock Tool and an authorized account.", parent=self.master_app.master):
            ...
            if self.master_app.log_panel: self.master_app.log_panel.log("Note: Xiaomi bootloader unlock often requires official Mi Unlock Tool and account authorization. This command attempts the generic unlock. It may fail or require specific drivers/device state.", "warning", include_timestamp=False)
        else:
...
class FileAdvancedTab(ttk.Frame):
    def __init__(self, parent_notebook, master_app: UltimateDeviceTool):
        ...
        ModernButton(file_col2, text=self.labels.get("btn_uninstall_app", "Uninstall App (Legacy)"), # Renamed to avoid confusion with App Manager
                                   command=self.action_uninstall_app_legacy, theme=self.theme, width=30).pack(pady=5, anchor=tk.W)
        ...
        # New button for pulling contacts
        ModernButton(file_col1, text=self.labels.get("btn_pull_contacts_vcf", "Pull Contacts (VCF)"),
                     command=self.action_pull_contacts_vcf, theme=self.theme, width=30).pack(pady=5, anchor=tk.W)
    ...
    def action_uninstall_app_legacy(self): # Renamed to distinguish from App Manager
    ...
    def action_backup_user_data(self):
        ...
        if not backup_file_path:
            ...
            return
        # -all includes all apps, -nosystem for user apps only, -apk to include apks
        command = ["adb", "backup", "-f", backup_file_path, "-apk", "-all", "-nosystem"] # Example: backup user apps + their APKs
    ...
    def action_pull_contacts_vcf(self):
        ...
        # This query attempts to get display names and phone numbers.
        # It might need adjustments for different Android versions or contact storage structures.
        # Data1 is typically the phone number for 'vnd.android.cursor.item/phone_v2' mimetype.
        adb_command = [
        ...
        def _contacts_callback(result):
            ...
            vcf_entries = []
            # Example line: Row: 0 display_name=John Doe, data1=+11234567890
            # Regex to parse: display_name=(.*?), data1=(.*)
            # Need to handle cases where display_name might be null or empty
            for line in stdout.splitlines():
                ...
                if number:
                    ...
                    # For simplicity, using display_name for FN. Parsing N correctly is more complex.
                    vcf_entries.append(f"FN:{name.strip()}")
                    vcf_entries.append(f"N:{name.strip()};;;;") # Basic N field
...
class AppManagerTab(ttk.Frame):
    def __init__(self, parent_notebook, master_app: UltimateDeviceTool):
        ...
        # Controls Frame (Filter, Search, Refresh)
        ...
        ModernButton(controls_frame, text=self.labels.get("btn_refresh_app_list", "Refresh"),
                     command=self.action_refresh_app_list, theme=self.theme, width=12, height=1, padx=5).pack(side=tk.RIGHT, padx=5)
        ...
        ModernButton(action_buttons_frame, text=self.labels.get("btn_copy_package_name", "Copy Package"),
                     command=self.action_copy_package_name, theme=self.theme, width=20, height=1).pack(side=tk.LEFT, padx=5, pady=5)
        ModernButton(action_buttons_frame, text=self.labels.get("btn_uninstall_selected_app", "Uninstall"),
                     command=self.action_uninstall_selected_app, theme=self.theme, width=20, height=1).pack(side=tk.LEFT, padx=5, pady=5)
        
        self.all_packages_cache = [] # Cache raw output for faster filtering
        self.action_refresh_app_list() # Initial load
    ...
    def action_refresh_app_list(self):
        ...
        def _list_apps_callback(result):
            ...
            self.filter_app_list_display() # Apply search filter
            if self.master_app.log_panel:
    ...
    def action_uninstall_selected_app(self):
        ...
        if messagebox.askyesno(self.labels.get("uninstall_app_confirm_title", "Confirm Uninstall"), confirm_msg, parent=self.master_app.master):
            ...
            # For system apps, might need: cmd = ["adb", "shell", "pm", "uninstall", "-k", "--user", "0", package_name]
            # but that's more complex and doesn't truly remove system apps without root.
            # Standard uninstall is safer.
            ...
            def _uninstall_callback(result):
                if result.get("return_code") == 0 and "Success" in result.get("stdout",""):
                    ...
                    self.action_refresh_app_list() # Refresh list after uninstall
                else:
                    if self.master_app.log_panel:
...
if __name__ == "__main__":
    ...
    try:
        ...
        _DEBUG_LOG_PATH = "application_debug_log_local.txt" # Fallback
        try:
            ...
            # At this point, logging to file might be impossible.
    ...
    try:
        ...
        try:
            ...
            # Consider showing a messagebox here if ADB is critical early on
        ...
        traceback.print_exc(file=open(_DEBUG_LOG_PATH, "a")) # Append traceback to log
        try:
            # Attempt to show a Tkinter error dialog if Tkinter itself hasn't failed
            root_err = tk.Tk()
            ...
            # If Tkinter dialog fails, print to stderr as a last resort
